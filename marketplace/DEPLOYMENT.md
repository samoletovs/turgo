...
This guide covers deploying the Marketplace to Azure and setting up CI/CD with GitHub Actions.
...
gh repo create marketplace --private --source=. --push

# Or manually:
# 1. Go to github.com → New Repository → name: "marketplace" → Private
...
az ad sp create-for-rbac \
  --name "sp-marketplace" \
  --role contributor \
  --scopes "/subscriptions/${SUBSCRIPTION_ID}/resourceGroups/rg-marketplace" \
  --sdk-auth
```
...
# Build and push
docker build -t YOUR_ACR_NAME.azurecr.io/marketplace:latest .
docker push YOUR_ACR_NAME.azurecr.io/marketplace:latest
...
1. Open the app URL from the script output: `https://marketplace.<hash>.azurecontainerapps.io`
...
# 4. Check progress at: github.com/YOUR_USERNAME/marketplace/actions
```
...
```bash
# ── Logs ──
az containerapp logs show -g rg-marketplace -n marketplace --type console --follow

# ── Restart ──
az containerapp revision restart -g rg-marketplace -n marketplace

# ── Scale ──
az containerapp update -g rg-marketplace -n marketplace --min-replicas 1 --max-replicas 5

# ── Update env vars ──
az containerapp update -g rg-marketplace -n marketplace \
  --set-env-vars "STRIPE_SECRET_KEY=sk_test_xxx"

# ── Connect to database ──
az postgres flexible-server connect \
  -n psql-marketplace -u pgadmin -d marketplace --interactive
...
| Issue | Solution |
|-------|----------|
| Container won't start | Check logs: `az containerapp logs show -g rg-marketplace -n marketplace --type console` |
