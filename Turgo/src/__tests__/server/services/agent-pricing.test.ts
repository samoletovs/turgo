...
describe("generatePriceAdjustSchedule", () => {
  ...
  it("dayOffsets are monotonically increasing", () => {
    ...
    for (let i = 1; i < schedule.length; i++) {
      expect(schedule[i].dayOffset).toBeGreaterThanOrEqual(
        schedule[i - 1].dayOffset,
      );
    }
  });
  ...
  it("target prices are monotonically decreasing", () => {
    ...
    for (let i = 1; i < schedule.length; i++) {
      expect(schedule[i].targetPrice).toBeLessThanOrEqual(
        schedule[i - 1].targetPrice,
      );
    }
  });
