...
describe("signInSchema", () => {
  ...
  it("rejects missing email", () =>
    expectFail(signInSchema, { password: "12345678" }));
  it("rejects missing password", () =>
    expectFail(signInSchema, { email: "a@b.com" }));
  it("rejects invalid email", () =>
    expectFail(signInSchema, { ...valid, email: "not-email" }));
  it("rejects short password", () =>
    expectFail(signInSchema, { ...valid, password: "123" }));
...
describe("registerSchema", () => {
  ...
  it("rejects missing name", () =>
    expectFail(registerSchema, { ...valid, name: undefined }));
  it("rejects short name", () =>
    expectFail(registerSchema, { ...valid, name: "A" }));
  it("rejects invalid email format", () =>
    expectFail(registerSchema, { ...valid, email: "bad" }));
  it("rejects short password", () =>
  ...
  it("accepts valid locale enum", () =>
    expectPass(registerSchema, { ...valid, locale: "lv" }));
  it("rejects invalid locale enum", () =>
    expectFail(registerSchema, { ...valid, locale: "fr" }));
});
...
describe("forgotPasswordSchema", () => {
  it("accepts valid email", () =>
    expectPass(forgotPasswordSchema, { email: "a@b.com" }));
  it("rejects invalid email", () =>
    expectFail(forgotPasswordSchema, { email: "nope" }));
...
describe("createListingSchema", () => {
  const valid = {
    ...
    description:
      "A really nice item that is in great condition and worth buying.",
    price: 99.99,
  ...
  it("rejects zero price", () =>
    expectFail(createListingSchema, { ...valid, price: 0 }));
  it("rejects negative price", () =>
    expectFail(createListingSchema, { ...valid, price: -10 }));
  it("accepts large price", () =>
    expectPass(createListingSchema, { ...valid, price: 1_000_000 }));
...
describe("updateListingSchema", () => {
  it("accepts empty object (all partial)", () =>
    expectPass(updateListingSchema, {}));
  it("accepts partial field", () =>
    expectPass(updateListingSchema, { price: 50 }));
  it("still validates field constraints", () =>
    expectFail(updateListingSchema, { price: -1 }));
});
...
describe("listingFilterSchema", () => {
  ...
  it("rejects invalid sortBy", () =>
    expectFail(listingFilterSchema, { sortBy: "random" }));
  it("rejects page < 1", () => expectFail(listingFilterSchema, { page: 0 }));
  it("rejects limit > 100", () =>
    expectFail(listingFilterSchema, { limit: 101 }));
  it("rejects negative minPrice", () =>
    expectFail(listingFilterSchema, { minPrice: -1 }));
  it("accepts condition filter", () =>
    expectPass(listingFilterSchema, { condition: "NEW" }));
  it("accepts status filter", () =>
    expectPass(listingFilterSchema, { status: "SOLD" }));
});
...
describe("updateAgentStatusSchema", () => {
  ...
  it("rejects missing agentId", () =>
    expectFail(updateAgentStatusSchema, { status: "ACTIVE" }));
});
...
describe("sendMessageSchema", () => {
  ...
  it("rejects empty content", () =>
    expectFail(sendMessageSchema, { ...valid, content: "" }));
...
describe("searchSchema", () => {
  ...
  it("rejects query over 200 chars", () =>
    expectFail(searchSchema, { query: "x".repeat(201) }));
  it("rejects negative minPrice", () =>
    expectFail(searchSchema, { query: "a", minPrice: -1 }));
});
...
describe("conciergeMessageSchema", () => {
  ...
  it("rejects empty message", () =>
    expectFail(conciergeMessageSchema, { message: "" }));
...
describe("sendOfferSchema", () => {
  ...
  it("rejects zero offerPrice", () =>
    expectFail(sendOfferSchema, { ...valid, offerPrice: 0 }));
