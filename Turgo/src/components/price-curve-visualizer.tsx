...
export function PriceCurveVisualizer({
  ...
  const { pathD, areaD, currentPoint, yTicks, xTicks } = useMemo(() => {
    ...
    // Build path
    const line = curve
      .map((pt, i) => `${i === 0 ? "M" : "L"} ${scaleX(pt.day).toFixed(1)} ${scaleY(pt.price).toFixed(1)}`)
      .join(" ");
    ...
    for (let i = 0; i < curve.length; i++) {
      if (curve[i].day >= currentDay) {
        // Interpolate
        if (i === 0) {
          cp = { x: scaleX(curve[0].day), y: scaleY(curve[0].price), price: curve[0].price, reason: curve[0].reason };
        } else {
        ...
        >
          ...
          {!compact && (
            <>
              <line
                x1={padding.left}
                y1={padding.top + chartH * (1 - (minPrice * 0.95 - minPrice * 0.95) / (startPrice * 1.05 - minPrice * 0.95))}
                x2={width - padding.right}
                y2={padding.top + chartH * (1 - (minPrice * 0.95 - minPrice * 0.95) / (startPrice * 1.05 - minPrice * 0.95))}
                stroke="#ef4444"
                strokeOpacity={0.3}
                strokeDasharray="6 3"
              />
          ...
          {!compact &&
            curve.map((pt, i) => {
              ...
              const y = padding.top + (1 - (pt.price - priceMin2) / (priceMax - priceMin2)) * chartH;
              return (
                <motion.circle
                  key={i}
                  cx={x}
          ...
          {priceEvents.map((evt, i) => {
            ...
            const color = evt.type === "sale" ? "#22c55e" : evt.type === "offer" ? "#3b82f6" : "#f97316";
            return (
              <g key={`evt-${i}`}>
