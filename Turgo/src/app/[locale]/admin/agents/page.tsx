...
export default function AgentsPage() {
  ...
  const stats = [
    ...
    },
    {
      ...
      bg: data.errorRate > 5 ? "bg-red-50 dark:bg-red-950" : "bg-green-50 dark:bg-green-950",
    },
    {
      title: "AI Cost Today",
          ...
          <CardContent>
            <div className="space-y-2">
              ...
              {Object.keys(data.sellingByStatus).length === 0 && (
                <p className="text-sm text-muted-foreground">No selling agents</p>
              )}
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <div className="space-y-2">
              ...
              {Object.keys(data.buyingByStatus).length === 0 && (
                <p className="text-sm text-muted-foreground">No buying agents</p>
              )}
            </div>
          </CardContent>
        </Card>
                  ...
                  {data.metricsHistory.map((m, i) => (
                    <tr key={i} className="hover:bg-muted/30">
                      ...
                      <td className="px-3 py-2">
                        <Badge variant={m.agentType === "SELLING" ? "default" : "secondary"}>
                          {m.agentType}
                        </Badge>
                      </td>
                      <td className="px-3 py-2 text-right">{m.processed}</td>
                      <td className="px-3 py-2 text-right">
                        <span className={m.errors > 0 ? "text-red-600 font-medium" : ""}>{m.errors}</span>
                      </td>
                      <td className="px-3 py-2 text-right">{m.avgMs}ms</td>
                      <td className="px-3 py-2 text-right">{m.tokens.toLocaleString()}</td>
                      <td className="px-3 py-2 text-right">€{(m.costCents / 100).toFixed(2)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
