...
export function NegotiationFlow({ messages, listingPrice, currency }: NegotiationFlowProps) {
  ...
  const lastOffer =
    steps.filter((s) => s.type === 'OFFER' || s.type === 'COUNTER_OFFER').pop()?.price ?? 0;
  const priceMovement = lastOffer - firstOffer;
  const priceMovementPercent = firstOffer > 0 ? ((lastOffer - firstOffer) / firstOffer) * 100 : 0;
          ...
          >
            <div
              className={cn(
                ...
                step.type === 'COUNTER_OFFER' && 'bg-orange-50 dark:bg-orange-950',
      ...
      {steps.length > 1 && !isResolved && (
        <div className="flex items-center gap-2 mt-2 text-[11px] text-muted-foreground">
          ...
          <span>
            ...
            {Math.abs(priceMovementPercent).toFixed(1)}%) over {steps.length} rounds
          </span>
        </div>
      )}
    </div>
