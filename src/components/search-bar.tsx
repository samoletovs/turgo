    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        if (Array.isArray(data)) {
          ...
          if (d.listings)
            results.push(...d.listings.map((l) => ({ ...l, type: 'listing' as const })));
          if (d.categories)
            results.push(
              ...d.categories.map((c) => ({
                text: c.text,
  ...
  return (
    <div className={cn('relative', className)}>
        ...
        <div className="relative flex-1">
          ...
          <Input
            ...
            aria-activedescendant={
              isOpen && selectedIndex >= 0 ? `search-option-${selectedIndex}` : undefined
            }
            className={cn(
              'pr-8',
      ...
      {isOpen && suggestions.length > 0 && (
        ...
        >
          <ul id="search-listbox" role="listbox" aria-label="Search suggestions" className="py-1">
            {suggestions.map((s, i) => (
              <li
                key={`${s.type}-${s.text}-${i}`}
                id={`search-option-${i}`}
              ...
              >
                ...
                >
                  ...
                  {s.type === 'category' && (
                    <span className="shrink-0 text-[10px] text-muted-foreground">Category</span>
                  )}
