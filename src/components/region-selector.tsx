...
const COUNTRY_CONFIG: Record<string, { label: string; flag: string; locale: Locale }> = {
...
export function RegionSelector({ currentLocale, className }: RegionSelectorProps) {
  ...
  return (
    <Select value={currentCountry} onValueChange={handleCountryChange}>
      <SelectTrigger className={`w-auto gap-1 border-none h-8 px-2 ${className || ''}`}>
