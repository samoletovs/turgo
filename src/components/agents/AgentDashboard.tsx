...
export function AgentDashboard({ locale, className }: AgentDashboardProps) {
  ...
  const { data: agents, isLoading: agentsLoading } = trpc.agent.myAgents.useQuery();
  const { data: stats, isLoading: statsLoading } = trpc.agent.dashboardStats.useQuery();
      ...
      {agentsLoading ? (
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="mt-1 text-sm text-muted-foreground/70">{t('noAgentsDesc')}</p>
          </CardContent>
        </Card>
      ) : (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
