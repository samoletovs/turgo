...
export async function handleAgentConfigInput(content: string, ctx: BuyingStepContext) {
  ...
  if (lower.includes('5') && lower.includes('min')) {
  ...
  } else if (lower.includes('daily') || lower.includes('day') || lower.includes('digest')) {
    ...
    if (!isNaN(num)) {
      const closest = MONITOR_FREQ.reduce((prev, curr) =>
        Math.abs(parseInt(curr.value, 10) - num) < Math.abs(parseInt(prev.value, 10) - num)
          ? curr
          : prev,
      );
...
export async function buildBuyingSummary(ctx: BuyingStepContext) {
  const strategyLabel = STRATEGY_LABELS[ctx.data.buyingStrategyId] || ctx.data.buyingStrategyId;
