...
const STRATEGY_FEATURES: Record<string, string[]> = {
  ...
  MAX_BID: ['proposalMonitor', 'proposalDealScore', 'proposalAutoOffer', 'proposalSellerAnalysis'],
  SNIPER: [
    'proposalMonitor',
    'proposalDealScore',
    'proposalAutoStrike',
...
export async function showBuyingAgentProposal(
  ...
  strategyId: 'TIME_ESCALATION' | 'MAX_BID' | 'SNIPER' | 'ACCEPT_LISTED' | 'EARLY_BIRD',
) {
  ctx.setCurrentStep('agent_proposal');

  const proposalKey = PROPOSAL_KEYS[strategyId] || 'proposalTimeEscalation';
  const featureKeys = STRATEGY_FEATURES[strategyId] || STRATEGY_FEATURES.TIME_ESCALATION;
  const canSkip = CAN_SKIP_AGENT[strategyId] ?? false;

  // Build message
  const features = featureKeys.map((k) => ctx.t(k)).join('\n');
  const message = features ? `${ctx.t(proposalKey)}\n${features}` : ctx.t(proposalKey);
