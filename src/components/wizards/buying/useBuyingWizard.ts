...
import { BUYING_STRATEGY_OPTIONS, handleBuyingStrategyAction } from './StrategyStep';
import { showBuyingAgentProposal, handleBuyingProposalAction } from './AgentProposalStep';
...
export function useBuyingWizard({
  ...
  const updateData = useCallback(
    (updates: Partial<BuyingWizardData>) => setData((prev) => ({ ...prev, ...updates })),
    [],
  );
  ...
  const addAgentMessage = useCallback((content: string, actions?: ChatAction[]) => {
    setMessages((prev) => [
      ...prev,
      {
  ...
  const createBuyingAgent = trpc.agent.createBuying.useMutation({
    onSuccess: (agent) => {
      setCurrentStep('done');
      addAgentMessage(t('agentLive', { frequency: String(data.monitorFrequency) }), [
  ...
  const handleAction = async (value: string) => {
    ...
    if (value === 'auto_all' || value === 'auto_monitor_only' || value === 'no_agent') {
