...
function getOptimalPostingTimeClient(categoryName: string): string {
  ...
  if (lower.includes('car') || lower.includes('vehicle') || lower.includes('transport'))
    return '🕐 Best time to post: Sunday evenings — car buyers are most active then';
  if (lower.includes('apartment') || lower.includes('real estate') || lower.includes('property'))
...
export async function handleAgentConfigInput(content: string, ctx: SellingStepContext) {
  let minP: number;
  ...
  const wasDefault = !content.toLowerCase().includes('no min') && (isNaN(parsed) || parsed <= 0);
  const defaultNote = wasDefault ? ctx.t('minPriceDefault', { price: String(minP) }) : '';
...
export function buildSellingSummary(ctx: SellingStepContext) {
  ...
  const urgencyLabel = URGENCY_OPTIONS.find((u) => u.value === ctx.data.urgency)?.label || '1 week';
  ...
  const strategyLabel = strategyNames[ctx.data.sellingStrategyId] || ctx.data.sellingStrategyId;

  // Generate price curve for visualization
  const curve = generatePriceCurveClient(ctx.data.price, minP, ctx.data.urgency);
