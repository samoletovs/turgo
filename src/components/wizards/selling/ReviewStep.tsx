...
export async function handlePublishAction(value: string, ctx: SellingStepContext) {
  // Validate required fields before submission
  const missing: string[] = [];
  let finalDescription = ctx.data.description?.trim() || '';
  if (!ctx.data.title || ctx.data.title.trim().length < 5) missing.push('title (min 5 chars)');
  if (finalDescription.length < 20) {
    if (ctx.data.title) {
      ...
      finalDescription =
        autoDesc.length >= 20 ? autoDesc : autoDesc + ' Contact seller for more details.';
  ...
  try {
    ...
    ctx.addAgentMessage(value === 'draft' ? ctx.t('draftSaved') : ctx.t('listingLive'), [
      {
      ...
      <Card>
        <CardContent className="flex items-start gap-3 p-4">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">{t('doneAutoRespondDesc')}</p>
          </div>
        </CardContent>
      </Card>
      <Card>
        <CardContent className="flex items-start gap-3 p-4">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">{t('doneSmartPricingDesc')}</p>
          </div>
        </CardContent>
      </Card>
      <Card>
        <CardContent className="flex items-start gap-3 p-4">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">{t('doneAutoNegotiateDesc')}</p>
          </div>
        </CardContent>
      </Card>
    </motion.div>
