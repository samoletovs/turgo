...
import { handleCategoryInput, handleCategoryAction, handleSubcategoryAction } from './CategoryStep';
...
import { SELLING_STRATEGY_OPTIONS, handleSellingStrategyAction } from './StrategyStep';
import { showSellingAgentProposal, handleSellingProposalAction } from './AgentProposalStep';
...
export function useSellingWizard({
  ...
  const updateData = useCallback(
    (updates: Partial<SellingWizardData>) => setData((prev) => ({ ...prev, ...updates })),
    [],
  );
  ...
  const resolveActionLabel = (value: string): string => {
    ...
    if (value.startsWith('strategy_')) {
      const s = SELLING_STRATEGY_OPTIONS.find((o) => o.value === value.replace('strategy_', ''));
    ...
    if (value.startsWith('urgency_')) {
      const u = URGENCY_OPTIONS.find((o) => o.value === value.replace('urgency_', ''));
  ...
  const handleAction = async (value: string) => {
    ...
    if (value === 'confirm_details') {
      ...
      await thinkAndRespond(t('whichCategory'), buildCategoryActions(categories, itemText, locale));
      return;
    }
  ...
  return {
    ...
    handleSendMessage,
    handleAction,
    onPhotoUpload: (e: ChangeEvent<HTMLInputElement>) => handlePhotoUpload(e, ctx),
