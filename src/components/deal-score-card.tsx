...
export function DealScoreCard({
  ...
  const grade = useMemo(() => getScoreGradeKey(breakdown.total), [breakdown.total]);
        ...
        <div className="relative flex h-12 w-12 shrink-0 items-center justify-center">
          <svg className="h-12 w-12 -rotate-90" viewBox="0 0 48 48">
            ...
            <motion.circle
              ...
              fill="none"
              stroke={
                breakdown.total >= 70 ? '#22c55e' : breakdown.total >= 50 ? '#eab308' : '#ef4444'
              }
              strokeWidth={3}
              strokeLinecap="round"
              ...
              animate={{
                strokeDashoffset: 2 * Math.PI * 20 - (breakdown.total / 100) * 2 * Math.PI * 20,
              }}
        ...
        <div className="mt-4 flex items-center gap-2 rounded-lg bg-muted/50 p-3">
          {breakdown.total >= 70 ? (
            <>
              ...
              <p className="text-xs">
                <strong className="text-green-600">{t('recommendation.buy')}</strong>
              </p>
            </>
          ) : breakdown.total >= 50 ? (
            <>
              ...
              <p className="text-xs">
                <strong className="text-yellow-600">{t('recommendation.watch')}</strong>
              </p>
            </>
          ) : (
            <>
              ...
              <p className="text-xs">
                <strong className="text-red-600">{t('recommendation.pass')}</strong>
              </p>
            </>
          )}
        </div>
