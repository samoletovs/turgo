      ...
      <div className="relative mx-auto max-w-7xl px-4">
        ...
        <div className="grid items-center gap-10 lg:grid-cols-2 lg:gap-16">
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <form
                ...
                action={mode === 'sell' ? `/${locale}/sell` : `/${locale}/search`}
                method="GET"
          ...
          <div className="flex justify-center lg:justify-end">
            <AnimatePresence mode="wait">
              {mode === 'sell' ? <SellVisual key="sell-visual" /> : <BuyVisual key="buy-visual" />}
            </AnimatePresence>
          </div>
        </div>
      </div>
      ...
      <DemoCard badgeLabel="SOLD">
        ...
        <div className="rounded-xl border bg-muted/30 p-4">
          <div className="flex gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-1 mt-1">
                ...
                <span className="text-xs text-muted-foreground">{t('hero.sell.aiOptimized')}</span>
              </div>
            </div>
          </div>
          ...
          <div className="mt-3 flex items-center gap-2">
            ...
            <span className="text-xs text-muted-foreground">{t('hero.demoListingCreated')}</span>
          </div>
        </div>
      </DemoCard>
    </motion.div>
      ...
      <DemoCard badgeLabel="MATCH">
        ...
        <div className="rounded-xl border bg-muted/30 p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-sm font-semibold text-primary">{t('hero.buy.matchFound')}</span>
          </div>
          ...
          <div className="mt-3 flex items-center justify-between rounded-lg bg-white/60 dark:bg-white/5 px-3 py-2">
            <span className="text-xs text-muted-foreground">{t('hero.buy.dealScore')}</span>
...
function DemoCard({ badgeLabel, children }: { badgeLabel: string; children: React.ReactNode }) {
  const t = useTranslations('home');
