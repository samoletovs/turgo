...
export function PricingTable({ plans, currentPlan, className }: PricingTableProps) {
  ...
  const buildFeatures = (name: PlanName) => {
    ...
    return [
      ...
      {
        label: limits.hasAiPremium ? tFeatures('premiumAgent') : tFeatures('basicAgent'),
        included: true,
      },
  ...
  const yearlySavings = Math.round(
    ((PLAN_PRICES.PRO.monthly * 12 - PLAN_PRICES.PRO.yearly) / (PLAN_PRICES.PRO.monthly * 12)) *
      100,
  );
    ...
    <div className={className}>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        {planConfigs.map(({ key, isPopular, tKey }) => {
          const price = interval === 'yearly' ? PLAN_PRICES[key].yearly : PLAN_PRICES[key].monthly;
