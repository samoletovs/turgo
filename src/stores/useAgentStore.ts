...
export const useAgentStore = create<AgentState & AgentActions>()((set) => ({
  ...
  addPendingApproval: () => set((state) => ({ pendingApprovals: state.pendingApprovals + 1 })),
  ...
  addUnreadUpdate: () => set((state) => ({ unreadAgentUpdates: state.unreadAgentUpdates + 1 })),
  ...
  handleAgentAction: (event) =>
    set((state) => ({
      ...
      ...(event.requiresApproval ? { pendingApprovals: state.pendingApprovals + 1 } : {}),
    })),
