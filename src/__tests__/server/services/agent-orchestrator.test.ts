...
describe('transitionAgent', () => {
  ...
  it('removes recurring jobs when transitioning to CANCELLED', async () => {
    mockGetRepeatableJobs.mockResolvedValue([{ name: 'agent-1-job', key: 'key-1' }]);
  ...
  it('removes recurring jobs when transitioning to COMPLETED', async () => {
    mockGetRepeatableJobs.mockResolvedValue([{ name: 'agent-1-summary', key: 'key-1' }]);
...
describe('scheduleJob', () => {
  ...
  it('supports delay option', async () => {
    await scheduleJob('buying-agents', 'buying-agent-monitor', { id: 'b1' }, { delay: 60000 });
  ...
  it('supports priority option', async () => {
    await scheduleJob('selling-agents', 'selling-agent-price-adjust', {}, { priority: 1 });
...
describe('scheduleRecurring', () => {
  it('adds a recurring job with CRON pattern', async () => {
    await scheduleRecurring('selling-agents', 'selling-agent-daily-summary', {}, '0 8 * * *');
...
describe('removeRecurringJobs', () => {
  ...
  it('does nothing when no matching jobs found', async () => {
    mockGetRepeatableJobs.mockResolvedValue([{ name: 'other-agent', key: 'k1' }]);
...
describe('state machine completeness', () => {
  const allStates: AgentState[] = ['ACTIVE', 'PAUSED', 'COMPLETED', 'CANCELLED'];
