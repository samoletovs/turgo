...
import { processConciergeMessage, detectLanguage } from '@/server/services/agent-concierge';
...
describe('processConciergeMessage', () => {
  ...
  describe('fallback intent detection (AI unavailable)', () => {
    ...
    it('detects support intent', async () => {
      const result = await processConciergeMessage('I have a problem with my account');
    ...
    it('returns localized responses for Latvian', async () => {
      const result = await processConciergeMessage('Gribu pārdot', undefined, 'lv');
    ...
    it('returns localized responses for Russian', async () => {
      const result = await processConciergeMessage('Продать машину', undefined, 'ru');
    ...
    it('returns localized responses for Lithuanian', async () => {
      const result = await processConciergeMessage('Noriu parduoti', undefined, 'lt');

      expect(result.intent).toBe('sell');
    });

    it('returns localized responses for Estonian', async () => {
      const result = await processConciergeMessage('Tahan müüa', undefined, 'et');
    ...
    it('problem/issue keywords trigger support intent', async () => {
      const result = await processConciergeMessage('I have a problem with login');
  ...
  describe('response structure', () => {
    ...
    it('always includes intent field', async () => {
      ...
      expect(['sell', 'buy', 'support', 'browse', 'other']).toContain(result.intent);
    });
