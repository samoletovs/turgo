...
describe('message.send', () => {
  ...
  it('throws when listing not found and no conversationId', async () => {
    ...
    const caller = createCaller(authedCtx());
    await expect(caller.message.send(validInput)).rejects.toThrow('Listing not found');
  });

  it('throws UNAUTHORIZED without session', async () => {
    const caller = createCaller(anonCtx());
    await expect(caller.message.send(validInput)).rejects.toThrow(/UNAUTHORIZED/);
  });

  it('rejects empty content', async () => {
    const caller = createCaller(authedCtx());
    await expect(caller.message.send({ ...validInput, content: '' })).rejects.toThrow();
  });
...
describe('message.sendOffer', () => {
  ...
  it('throws UNAUTHORIZED without session', async () => {
    const caller = createCaller(anonCtx());
    await expect(caller.message.sendOffer(validInput)).rejects.toThrow(/UNAUTHORIZED/);
  });

  it('rejects non-positive offerPrice', async () => {
    const caller = createCaller(authedCtx());
    await expect(caller.message.sendOffer({ ...validInput, offerPrice: 0 })).rejects.toThrow();
  });

  it('rejects negative offerPrice', async () => {
    const caller = createCaller(authedCtx());
    await expect(caller.message.sendOffer({ ...validInput, offerPrice: -50 })).rejects.toThrow();
  });
});
...
describe('message.approveMessage', () => {
  ...
  it('throws UNAUTHORIZED without session', async () => {
    const caller = createCaller(anonCtx());
    await expect(caller.message.approveMessage({ messageId: validCuid })).rejects.toThrow(
      /UNAUTHORIZED/,
    );
  });
...
describe('message.rejectMessage', () => {
  ...
  it('throws UNAUTHORIZED without session', async () => {
    const caller = createCaller(anonCtx());
    await expect(caller.message.rejectMessage({ messageId: validCuid })).rejects.toThrow(
      /UNAUTHORIZED/,
    );
  });
