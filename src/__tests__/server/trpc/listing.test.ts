...
describe('listing.create', () => {
  const validInput = {
    ...
    description: 'A really nice item that is in great condition and worth buying right now.',
    price: 150,
  ...
  it('throws UNAUTHORIZED without session', async () => {
    ...
    await expect(caller.listing.create(validInput)).rejects.toThrow(/UNAUTHORIZED/);
  });

  it('throws on invalid input (short title)', async () => {
    const caller = createCaller(authedCtx());
    await expect(caller.listing.create({ ...validInput, title: 'Hi' })).rejects.toThrow();
  });

  it('throws on invalid input (negative price)', async () => {
    const caller = createCaller(authedCtx());
    await expect(caller.listing.create({ ...validInput, price: -5 })).rejects.toThrow();
  });
});
...
describe('listing.getById', () => {
  ...
  it('rejects invalid id format', async () => {
    const caller = createCaller(anonCtx());
    await expect(caller.listing.getById({ id: 'not-a-cuid' })).rejects.toThrow();
  });
});
