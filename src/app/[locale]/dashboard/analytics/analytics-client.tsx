...
function _MiniLineChart({ data, color = '#2563EB' }: { data: number[]; color?: string }) {
...
export function AnalyticsClient({ locale: _locale }: AnalyticsClientProps) {
  ...
  const stats = useMemo(() => {
    ...
    const totalMessages = listings.reduce((sum, l) => sum + (l._count?.messages ?? 0), 0);
    const activeCount = listings.filter((l) => l.status === 'ACTIVE').length;
    const conversionRate = totalViews > 0 ? ((totalMessages / totalViews) * 100).toFixed(1) : '0';

    // Simulated views over time (last 7 days from current view counts)
    const viewsOverTime = randomFactors.map((factor) => Math.round((totalViews / 7) * factor));
    ...
    <div className="container mx-auto px-4 py-8">
        ...
        <Card>
          <CardContent className="flex items-center gap-4 p-4">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">{t('totalInquiries')}</p>
            </div>
          </CardContent>
        </Card>
      ...
      <div className="mb-8 grid gap-6 lg:grid-cols-2">
        ...
        <Card>
          ...
          <CardContent>
            <BarChart data={stats.viewsOverTime} labels={dayLabels} color="#2563EB" />
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="flex flex-col gap-4">
              ...
              <div className="flex items-center justify-between">
                <span className="text-sm text-muted-foreground">{t('totalInquiries')}</span>
                <span className="font-mono text-sm font-bold">{stats.totalMessages}</span>
              </div>
              <div className="h-2 w-full overflow-hidden rounded-full bg-muted">
                <div
                  className="h-full rounded-full bg-green-500 transition-all"
                  style={{
                    width: `${Math.min(parseFloat(stats.conversionRate) * 5, 100)}%`,
                  }}
                />
              </div>
          ...
          {stats.topListings.length === 0 ? (
            ...
            <div className="divide-y">
              {stats.topListings.map((listing, idx) => {
                ...
                const rate = views > 0 ? ((messages / views) * 100).toFixed(1) : '0';
