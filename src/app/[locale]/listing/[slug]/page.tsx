        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="relative">
            ...
            <div className="absolute left-4 top-4 z-10 flex gap-2 pointer-events-none">
              {isFeatured && <Badge className="bg-yellow-500 text-white">{t('featured')}</Badge>}
          ...
          <div className="rounded-lg border p-4">
            <div className="flex items-baseline justify-between">
              ...
              <div className="flex items-center gap-2">
                {listing.negotiable && <Badge variant="outline">{t('negotiable')}</Badge>}
        ...
        <div className="space-y-6">
          ...
          {isOwner && (
            <Card className="border-orange-200 bg-orange-50/50 dark:border-orange-900 dark:bg-orange-950/20">
              <CardHeader>
                <CardTitle className="text-lg">{t('manageYourListing')}</CardTitle>
              </CardHeader>
          ...
          {listing.sellingAgent && isOwner && (
            <Card className="border-blue-200 bg-blue-50/50 dark:border-blue-900 dark:bg-blue-950/20">
              <CardHeader>
                ...
                <p className="text-xs text-muted-foreground">{t('agentInsightsDesc')}</p>
              </CardHeader>
              <CardContent className="space-y-4 text-sm">
                ...
                <div className="flex items-center justify-between">
                  <span className="text-muted-foreground">{t('status')}</span>
                  <Badge
                    variant={listing.sellingAgent.status === 'ACTIVE' ? 'success' : 'secondary'}
                  >
                    {listing.sellingAgent.status === 'ACTIVE' ? t('active') : t('agentPaused')}
                  </Badge>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-muted-foreground">{t('agentStrategy')}</span>
                ...
                <div className="grid grid-cols-2 gap-3 rounded-lg border p-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="text-xs text-muted-foreground">{t('agentViews')}</p>
                      <p className="font-semibold">{listing.sellingAgent.totalViews}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <MessageSquare className="h-4 w-4 text-muted-foreground" />
                    <div>
                      <p className="text-xs text-muted-foreground">{t('agentInquiries')}</p>
                      <p className="font-semibold">{listing.sellingAgent.totalInquiries}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <Target className="h-4 w-4 text-muted-foreground" />
                    <div>
                      <p className="text-xs text-muted-foreground">{t('agentOffers')}</p>
                      <p className="font-semibold">{listing.sellingAgent.totalOffers}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <Zap className="h-4 w-4 text-muted-foreground" />
                    <div>
                      <p className="text-xs text-muted-foreground">{t('agentBestOffer')}</p>
                ...
                <div className="flex items-center justify-between rounded-lg border p-3">
                  <div>
                    <p className="text-xs text-muted-foreground">{t('agentCurrentPrice')}</p>
                    <p className="text-lg font-bold text-primary">
                      {formatPrice(listing.sellingAgent.currentPrice)}
                    </p>
                  </div>
                  <div className="text-right">
                    <p className="text-xs text-muted-foreground">{t('agentMinPrice')}</p>
