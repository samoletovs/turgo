...
import { Rocket, TrendingUp, ShoppingCart, CreditCard, Shield, Target } from 'lucide-react';
        ...
        return (
          <div key={section.id}>
            <div className="flex items-center gap-3 mb-4">
              ...
              <h2 className="text-xl font-semibold">{t(`sections.${section.id}.title`)}</h2>
            </div>
            <Accordion type="single" className="rounded-xl border divide-y">
              {section.keys.map((key) => (
                <AccordionItem key={key} value={`${section.id}-${key}`} className="border-0 px-4">
                  <AccordionTrigger className="text-left font-medium">
                    {t(`sections.${section.id}.${key}.q`)}
                  </AccordionTrigger>
                  <AccordionContent>{t(`sections.${section.id}.${key}.a`)}</AccordionContent>
                </AccordionItem>
              ))}
            </Accordion>
          </div>
