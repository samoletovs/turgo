...
function ReviewForm({ revieweeId, onSuccess }: { revieweeId: string; onSuccess: () => void }) {
            ...
            <CardContent className="pt-6 text-center">
              ...
              <div className="mt-4 grid grid-cols-2 gap-4 border-t pt-4">
                <div className="flex flex-col items-center gap-1">
                  ...
                  <span className="text-lg font-bold">{user._count.listings}</span>
                  <span className="text-xs text-muted-foreground">{t('listings')}</span>
                </div>
                <div className="flex flex-col items-center gap-1">
                  <Star className="h-5 w-5 text-muted-foreground" />
                  <span className="text-lg font-bold">{user._count.reviewsReceived}</span>
          ...
          {!reviews || reviews.reviews.length === 0 ? (
            <Card>
              <CardContent className="py-12 text-center">
                ...
                <h3 className="mb-2 text-lg font-semibold">{tr('noReviews')}</h3>
                <p className="text-muted-foreground">{tr('noReviewsDesc')}</p>
              </CardContent>
            </Card>
          ) : (
            <div className="space-y-4">
              {reviews.reviews.map((review: (typeof reviews.reviews)[number]) => (
                <Card key={review.id}>
                  <CardContent className="pt-6">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center justify-between">
                          <p className="font-medium">{review.reviewer.name || 'User'}</p>
