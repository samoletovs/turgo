    ...
    switch (event.type) {
      case 'checkout.session.completed': {
        ...
        if (boostType && session.metadata?.listingId) {
          ...
          const durationDays =
            boostConfig?.durationDays || parseInt(session.metadata?.durationDays || '7', 10);
      ...
      case 'customer.subscription.updated': {
        ...
        if (existingSub) {
          const periodStart = (subscription as unknown as Record<string, number>)
      ...
      case 'invoice.payment_failed': {
        ...
        const subscriptionId = (invoice as unknown as Record<string, unknown>).subscription as
          | string
          | undefined;
        if (subscriptionId) {
          const existingSub = await db.subscription.findFirst({
