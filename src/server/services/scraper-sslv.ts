    ...
    if (config.subcategories) {
      for (const [subSlug, subUrl] of Object.entries(config.subcategories)) {
        try {
          ...
          console.error(`[Scraper] Error processing ${slug}/${subSlug}:`, error);
          errors++;
        }
      }
    }
...
async function upsertSnapshot(
  ...
  try {
    ...
    if (previousSnapshot && previousSnapshot.listingCount > 0 && previousSnapshot.medianPrice) {
      const countChange =
        (stats.listingCount - previousSnapshot.listingCount) / previousSnapshot.listingCount;
      const priceChange =
        (stats.medianPrice - previousSnapshot.medianPrice) / previousSnapshot.medianPrice;
