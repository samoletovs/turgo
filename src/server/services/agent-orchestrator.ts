...
export function getQueue(name: string): Queue {
  if (!queues.has(name)) {
    queues.set(name, new Queue(name, { connection: getConnection() as ConnectionOptions }));
  }
...
export function registerWorker(queueName: string, processor: (job: Job) => Promise<void>): Worker {
...
export async function initializeOrchestrator() {
  ...
  await scheduleRecurring('selling-agents', 'selling-agent-price-adjust', {}, '0 */2 * * *');
  ...
  console.log('[Agent Orchestrator] Initialized — workers registered, CRON jobs scheduled');
}
