...
import type { MarketContext, PriceTrendDirection, StrategyRecommendation } from '@/types';
 ...
 */
const SELLING_SIGNAL_SCORES: Record<string, Record<SellingStrategyId, number>> = {
  // Supply level
...
function classifyTimeOnMarket(avgDays: number | null): 'fast' | 'moderate' | 'slow' {
...
export async function recommendSellingStrategy(input: {
  ...
  const ctx = await buildMarketContext(input.categoryId, input.locationId, input.subcategorySlug);
  ...
  const strategies: SellingStrategyId[] = ['SEALED_BID', 'FIXED_PRICE', 'DUTCH_AUCTION'];
  ...
  for (const strategy of strategies) {
    ...
    score += (SELLING_SIGNAL_SCORES[signals[0]]?.[strategy] ?? 0.5) * weights.supply;
    score += (SELLING_SIGNAL_SCORES[signals[1]]?.[strategy] ?? 0.5) * weights.spread;
    score += (SELLING_SIGNAL_SCORES[signals[2]]?.[strategy] ?? 0.5) * weights.time;
    score += (SELLING_SIGNAL_SCORES[signals[3]]?.[strategy] ?? 0.5) * weights.trend;
...
function buildSellingReasoning(
  ...
  parts.push(`Based on ${listingLabel} (median €${ctx.medianPrice.toFixed(0)})`);

  if (strategy === 'SEALED_BID') {
    if (ctx.supplyLevel === 'low') parts.push('low supply creates competitive demand');
    ...
    if (ctx.priceTrend.direction === 'stable') parts.push('stable prices indicate a mature market');
    if (ctx.avgDaysToSell !== null && ctx.avgDaysToSell <= 7)
      parts.push('items sell quickly at fair prices');
  } else if (strategy === 'DUTCH_AUCTION') {
    if (ctx.supplyLevel === 'high') parts.push('high supply means you need to stay competitive');
...
type BuyingStrategyId = 'TIME_ESCALATION' | 'MAX_BID' | 'SNIPER' | 'ACCEPT_LISTED' | 'EARLY_BIRD';
...
export async function recommendBuyingStrategy(input: {
  ...
  // Budget ratio
  const budgetRatio = ctx.medianPrice > 0 ? input.maxBudget / ctx.medianPrice : 1;
  ...
  for (const strategy of strategies) {
    ...
    score += (BUYING_SIGNAL_SCORES[signals[0]]?.[strategy] ?? 0.5) * weights.supply;
    score += (BUYING_SIGNAL_SCORES[signals[1]]?.[strategy] ?? 0.5) * weights.spread;
    score += (BUYING_SIGNAL_SCORES[signals[2]]?.[strategy] ?? 0.5) * weights.time;
    score += (BUYING_SIGNAL_SCORES[signals[3]]?.[strategy] ?? 0.5) * weights.trend;
...
function buildBuyingReasoning(
  ...
  parts.push(`Analyzed ${listingLabel} (median €${ctx.medianPrice.toFixed(0)})`);

  if (strategy === 'TIME_ESCALATION') {
    if (ctx.supplyLevel === 'high') parts.push('plenty of options, so starting low is safe');
    if (ctx.priceTrend.direction === 'falling')
    ...
    if (ctx.supplyLevel === 'low') parts.push('limited supply means items go fast');
    if (ctx.priceTrend.direction === 'rising')
      parts.push('prices are rising — buy before they go higher');
  } else if (strategy === 'SNIPER') {
    if (ctx.avgDaysToSell !== null && ctx.avgDaysToSell > 14)
      parts.push('many listings sit unsold — sellers grow flexible near expiry');
    if (ctx.priceTrend.direction !== 'rising') parts.push('no upward pressure, safe to wait');
  } else if (strategy === 'ACCEPT_LISTED') {
    if (ctx.priceSpread === 'tight') parts.push('prices are narrow — listed price is fair');
    if (ctx.supplyLevel === 'low')
      parts.push('scarce items — paying asking price secures the deal');
  } else if (strategy === 'EARLY_BIRD') {
    if (ctx.priceSpread === 'wide') parts.push('wide price range means room to negotiate down');
    if (ctx.supplyLevel === 'high') parts.push('high supply gives leverage for an early low offer');
  }

  if (budgetRatio < 0.8)
    parts.push('your budget is below market median — a bargain strategy is key');
  else if (budgetRatio > 1.2) parts.push('your budget exceeds the median — you can move quickly');
