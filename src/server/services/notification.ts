...
export async function sendAgentSummaryEmail(params: {
  ...
  const recommendationList = params.recommendations.map((r) => `<li>${r}</li>`).join('');
...
export async function checkAndNotifySavedSearchMatches(): Promise<void> {
  ...
  for (const search of searches) {
    ...
    if (filters.condition) where.condition = filters.condition as Prisma.EnumListingConditionFilter;
