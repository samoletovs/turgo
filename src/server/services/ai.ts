...
export async function aiEmbed(texts: string[], userTier?: UserTier): Promise<AiEmbeddingResult> {
  const provider = resolveProvider(userTier);
...
export function createMessages(systemPrompt: string, userMessage: string): AiChatMessage[] {
  return [
