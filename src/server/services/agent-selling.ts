...
export async function calculateOptimalPrice(params: {
  ...
  const stats = await getMarketStats(params.categoryId, params.locationId, params.subcategorySlug);
  const trend = await getPriceTrend(params.categoryId, params.locationId, params.subcategorySlug);
...
export function shouldAdjustPrice(agent: {
  ...
  if (progress > 0.3 && viewToInquiryRatio < 0.02 && agent.currentPrice > agent.minimumPrice) {
    ...
    const newPrice = Math.max(agent.minimumPrice, Math.round(agent.currentPrice * (1 - reduction)));
...
async function buildSellingAgentContext(agentId: string): Promise<SellingAgentContext | null> {
...
export async function processIncomingOffer(params: {
  ...
  const { sellingAgentId, listingId, buyerId, buyingAgentId, offerPrice, message } = params;
  ...
  const strategyId: SellingStrategyId = agent?.sellingStrategyId ?? 'SEALED_BID';
 ...
 */
export async function getSellerOffers(sellingAgentId: string): Promise<SellerOfferView[]> {
  const agentCtx = await buildSellingAgentContext(sellingAgentId);
  if (!agentCtx) return [];
...
export async function checkDeadlines(): Promise<number> {
  ...
  for (const agent of agents) {
    ...
    const hoursLeft = (agent.listing.expiresAt.getTime() - Date.now()) / (1000 * 60 * 60);
    ...
    const deadlineAction = await strategy.onDeadlineApproaching(ctx, offerContexts);
    if (deadlineAction) {
      await db.agentAction.create({
        data: {
          sellingAgentId: agent.id,
...
export type { BuyerOfferAck, SellerOfferView, OfferContext, TickAction, DeadlineAction };
...
export function shouldBoost(params: {
  ...
  const { totalViews, totalInquiries, daysActive, urgency, currentlyBoosted, previousBoosts } =
    params;
...
export async function generateDailySummary(agentId: string): Promise<DailySummary | null> {
  ...
  if (todayActions.length === 0) {
    ...
    if (responses.length > 0)
      highlights.push(`Answered ${responses.length} buyer question(s) automatically`);
  ...
  const changePercent =
    originalPrice > 0 ? ((Number(agent.listing.price) - originalPrice) / originalPrice) * 100 : 0;
