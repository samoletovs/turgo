...
import { createNotification, sendPushNotification, sendAgentSummaryEmail } from './notification';
...
async function processSellingPriceAdjust(job: Job): Promise<void> {
  ...
  for (const agentId of agentIds) {
    try {
      ...
      console.error(`[Worker] Price adjust failed for agent ${agentId}:`, error);
    }
  }
}
...
async function processSellingDailySummary(job: Job): Promise<void> {
  ...
  for (const agent of agents) {
    try {
      ...
      console.error(`[Worker] Daily summary failed for agent ${agent.id}:`, error);
    }
  }
}
  ...
  for (const agentId of agentIds) {
    try {
      ...
      if (matchCount > 0) {
        ...
        console.log(`[Worker] Agent ${agentId}: found ${matchCount} new matches`);
      }
    } catch (error) {
      console.error(`[Worker] Monitor failed for buying agent ${agentId}:`, error);
    }
  }
}
...
async function processBuyingAgentAutoNegotiate(job: Job): Promise<void> {
  console.log(`[Worker] Processing buying-agent-auto-negotiate (strategy-based)`, job.data);
