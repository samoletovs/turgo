...
export const agentRouter = createTRPCRouter({
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const agent = await ctx.db.sellingAgent.create({
        data: {
          ...
          strategyConfig: (input.strategyConfig as Prisma.InputJsonValue) ?? undefined,
          status: 'ACTIVE',
        },
      });
  ...
  getActions: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      const actions = await ctx.db.agentAction.findMany({
        where: {
          OR: [{ sellingAgentId: input.agentId }, { buyingAgentId: input.agentId }],
        },
        orderBy: { createdAt: 'desc' },
        take: input.limit + 1,
  ...
  submitOffer: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      const { processIncomingOffer } = await import('@/server/services/agent-selling');
  ...
  listOffers: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      const { getSellerOffers } = await import('@/server/services/agent-selling');
  ...
  getBoostRecommendation: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      return shouldBoost({
        ...
        currentlyBoosted: agent.listing.boosts?.some((b) => b.endAt > new Date()) ?? false,
