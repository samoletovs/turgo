import { createTRPCRouter, protectedProcedure, tieredProcedure } from '@/server/trpc';
...
export const aiRouter = createTRPCRouter({
  ...
  concierge: protectedProcedure.input(conciergeMessageSchema).mutation(async ({ input }) => {
...
${input.attributes ? `Attributes: ${JSON.stringify(input.attributes)}` : ''}`,
      ...
      const result = await aiComplete({ messages, temperature: 0.7, maxTokens: 300 }, userTier);
