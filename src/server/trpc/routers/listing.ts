...
export const listingRouter = createTRPCRouter({
  ...
  list: publicProcedure.input(listingFilterSchema).query(async ({ ctx, input }) => {
  ...
  update: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (sanitizedData.address && !sanitizedData.latitude && !sanitizedData.longitude) {
      ...
      z.object({
        ...
        agent: z
          .object({
            ...
            urgency: z
              .enum(['ONE_DAY', 'THREE_DAYS', 'ONE_WEEK', 'TWO_WEEKS', 'ONE_MONTH', 'NO_RUSH'])
              .default('ONE_WEEK'),
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const listing = await ctx.db.listing.create({
        data: {
          ...
          expiresAt:
            input.status === 'ACTIVE' ? new Date(Date.now() + 30 * 24 * 60 * 60 * 1000) : undefined,
        },
      });
