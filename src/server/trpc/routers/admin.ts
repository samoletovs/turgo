...
export const adminRouter = createTRPCRouter({
  ...
  revenue: adminProcedure.query(async ({ ctx }) => {
    return cached('admin:revenue', async () => {
      ...
      const subsByMonth = await ctx.db.$queryRaw<{ month: Date; cnt: bigint }[]>(Prisma.sql`
      ...
      const boostsByMonth = await ctx.db.$queryRaw<{ month: Date; cnt: bigint }[]>(Prisma.sql`
        SELECT DATE_TRUNC('month', m.month) AS month,
               COUNT(b.id)                  AS cnt
        FROM   generate_series(
                 ${twelveMonthsAgo}::timestamp,
  ...
  agentMonitoring: adminProcedure.query(async ({ ctx }) => {
    ...
    return {
      ...
      sellingByStatus: Object.fromEntries(sellingByStatus.map((s) => [s.status, s._count.status])),
      buyingByStatus: Object.fromEntries(buyingByStatus.map((s) => [s.status, s._count.status])),
    };
  }),
  ...
  escalations: adminProcedure
    .input(
      z.object({
        ...
        source: z
          .enum(['SELLING_AGENT', 'BUYING_AGENT', 'CONCIERGE', 'AUTO_MODERATION'])
          .optional(),
        page: z.number().min(1).default(1),
        limit: z.number().min(1).max(100).default(20),
      }),
